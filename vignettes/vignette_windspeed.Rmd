...
fit_space     <- fit_model(windspeed[inds], locs[inds,], X[inds,], 
    covfun_name = "arma_matern_sphere", silent = TRUE)
fit_spacetime <- fit_model(windspeed[inds], locstime[inds,], X[inds,], 
    covfun_name = "arma_matern_spheretime", silent = TRUE)
fit_space
fit_spacetime
```
...
# predictions
pred <- predictions(fit_spacetime$covparms, "arma_matern_spheretime", windspeed,
    locstime, locstime_pred, X, X_pred, fit_spacetime$betahat, m = 40)
# plot predictions
par(mar=c(4,4,1,1))
pred_array <- array( pred, c(length(longrid),length(latgrid)) )
fields::image.plot(longrid,latgrid,pred_array)
# conditional simulations
sim <- cond_sim(fit_spacetime$covparms, "arma_matern_spheretime", windspeed,
    locstime, locstime_pred, X, X_pred, fit_spacetime$betahat, m = 40)
