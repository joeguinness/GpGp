...
```{r}
set.seed(1234) # for predictable behavior on CRAN checks
library("GpGp")
data(jason3)
head(jason3)
lat       <- jason3$lat
...
The two models take about 40 seconds to fit on my system.
...
fit_space     <- fit_model(windspeed[inds], locs[inds,], X[inds,], 
    covfun_name = "exponential_sphere", silent = TRUE)
t2 <- proc.time()
fit_spacetime <- fit_model(windspeed[inds], locstime[inds,], X[inds,], 
    covfun_name = "exponential_spheretime", silent = TRUE)
t3 <- proc.time()
summary(fit_space)
...
pred <- predictions(fit = fit_spacetime, 
    ...
    locs_pred = locstime_pred, X_pred = X_pred, m = 80)
# plot predictions
par(mar=c(4,4,1,1))
